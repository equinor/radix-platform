locals {
  ...
  grafana_uris_azuread = [
    for k, v in module.clusters.oidc_issuer_url :
    "https://grafana.${k}.c2.radix.equinor.com/login/azuread"
  ]
}
...
module "grafana_redirect_uris" {
  ...
  type           = "Web"
  redirect_uris  = concat(["https://grafana.c2.radix.equinor.com/login/generic_oauth"], ["https://grafana.c2.radix.equinor.com/login/azuread"], local.grafana_uris, local.grafana_uris_azuread)
}
