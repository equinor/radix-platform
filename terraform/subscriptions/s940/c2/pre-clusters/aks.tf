...
locals {
  ...
  # Node OS maintenance runs on the third Thursday of the month
  node_os_maintenance_week_index  = "Third"
...
module "aks" {
  ...
  hostencryption                    = lookup(module.config.cluster[each.key], "hostencryption", true)
