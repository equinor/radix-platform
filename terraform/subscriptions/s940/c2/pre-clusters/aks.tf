...
module "aks" {
  ...
  hub_to_cluster_peering_name       = lookup(module.config.cluster[each.key], "hub_to_cluster_peering", "hub-to-vnet-${each.key}") #"hub-to-vnet-${each.key}"
  cluster_to_hub_peering_name       = lookup(module.config.cluster[each.key], "cluster_to_hub_peering", "vnet-${each.key}-to-hub") #"vnet-${each.key}-to-hub"
  dnszones                          = keys(module.config.private_dns_zones_names) # Extract only the DNS zone names (map keys) from module.config.private_dns_zones_names.
  dns_zone_resolution_policies      = { for name, zone in module.config.private_dns_zones_names : name => zone.resolution_policy }
