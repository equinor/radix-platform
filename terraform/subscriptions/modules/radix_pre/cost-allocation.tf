...
data "azurerm_user_assigned_identity" "cost-allocation-writer" {
  resource_group_name = "cost-allocation-${var.environment}"
...
module "cost-allocation-writer" {
  ...
  name                = "k8s-radix-cost-allocation-writer-${each.key}-${var.environment}"
  issuer              = each.value
...
module "cost-allocation-api-reader-prod" {
  source              = "../../modules/federated-credentials"
  for_each            = module.clusters.oidc_issuer_url
  name                = "k8s-radix-cost-allocation-reader-prod-${each.key}"
  issuer              = each.value
...
module "cost-allocation-api-reader-qa" {
  source              = "../../modules/federated-credentials"
  for_each            = module.clusters.oidc_issuer_url
  name                = "k8s-radix-cost-allocation-reader-qa-${each.key}"
  issuer              = each.value
