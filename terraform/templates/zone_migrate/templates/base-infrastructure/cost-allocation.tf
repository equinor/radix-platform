...
module "mssql-database" {
  ...
  server_name                 = "sql-radix-cost-allocation-${prefix}{module.config.environment}"
  managed_identity_admin_name = "radix-id-cost-allocation-admin-${prefix}{module.config.environment}"
...
module "github-workload-id" {
  ...
  name                = "radix-id-cost-allocation-github-${prefix}{module.config.environment}"
  ...
  federated_credentials = {
    github-main = {
      name    = "gh-radix-cost-allocation-acr-main-${prefix}{module.config.environment}"
      issuer  = "https://token.actions.githubusercontent.com"
      subject = "repo:equinor/radix-cost-allocation:ref:refs/heads/release"
    }
  }
