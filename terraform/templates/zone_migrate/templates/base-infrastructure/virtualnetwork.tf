...
module "azurerm_public_ip_prefix_ingress" {
  ...
  publicipprefixname  = "ippre-ingress-radix-aks-${destinationzone}-${location}-001" # template
...
module "azurerm_public_ip_prefix_egress" {
  ...
  location            = module.config.location
  resource_group_name = module.resourcegroup_clusters.data.name
  publicipprefixname  = "ippre-radix-aks-${destinationzone}-${location}-001" # template
  pipprefix           = "radix-aks"
  pippostfix          = module.config.location
  enviroment          = destinationzone # template
  prefix_length       = 28
  publicipcounter     = 16
}
