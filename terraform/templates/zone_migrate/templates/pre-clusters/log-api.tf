...
module "log-api-mi-prod" {
  ...
  name                = "k8s-radix-log-api-prod-${prefix}{each.key}-${prefix}{module.config.environment}"
  issuer              = each.value
...
module "log-api-mi-qa" {
  source              = "../../../modules/federated-credentials"
  for_each            = module.clusters.oidc_issuer_url
  name                = "k8s-radix-log-api-qa-${prefix}{each.key}-${prefix}{module.config.environment}"
  issuer              = each.value
