...
data "azurerm_subnet" "virtual_subnets" {
  for_each             = {
    for key, value in var.resource_groups : key => valueif length(regexall("cluster-vnet-hub", key)) > 0
  }
  name                 = "private-links"
...
resource "azurerm_private_endpoint" "northeurope" {
  for_each            = {
    for key in compact([
      ...
      value.location == var.AZ_LOCATION && endswith(key, "prod") && value.private_endpoint ? key : ""]) : key =>
...
resource "azurerm_private_endpoint" "westeurope" {
  for_each            = {
    for key in compact([
      for key, value in local.storageaccount_private_subnet :
      value.location == "westeurope" && endswith(key, "c2") && value.private_endpoint ? key : ""]) : key =>
    local.storageaccount_private_subnet[key]
  }
  name                = each.key
  resource_group_name = each.value["resource_group_name"]
...
resource "azurerm_private_dns_a_record" "dns_a_northeurope" {
  for_each            = {
    for key in compact([
      for key, value in local.privatelink_dns_record :
      endswith(key, "prod") && endswith(value.name, "prod") && value.private_endpoint ? key : ""]) : key =>
...
resource "azurerm_private_dns_a_record" "dns_a_westeurope" {
  for_each            = {
    for key in compact([
      for key, value in local.privatelink_dns_record :
      endswith(key, "c2") && endswith(value.name, "c2") && value.private_endpoint ? key : ""]) : key =>
    local.privatelink_dns_record[key]
  }
  name                = each.value["name"]
  zone_name           = "privatelink.blob.core.windows.net"
...
resource "azurerm_role_assignment" "westeurope" {
  for_each             = {
    for key in compact([
      for key, value in var.storage_accounts :
      value.backup_center && value.location == "westeurope" && value.kind == "StorageV2" ? key : ""]) : key =>
    var.storage_accounts[key]
  }
  scope                = var.storage_accounts[each.key].create_with_rbac ? data.azurerm_storage_account.storageaccounts[each.key].id : azurerm_storage_account.storageaccounts[each.key].id
  role_definition_name = "Storage Account Backup Contributor"
