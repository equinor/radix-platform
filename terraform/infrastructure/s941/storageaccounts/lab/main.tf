...
#   value = resource.azurerm_role_assignment.az_roleassignemnt
# }


resource "azurerm_role_assignment" "northeurope" {
  for_each             = { for key in compact([for key, value in var.storage_accounts : value.backup_center ? key : false && value.location == "northeurope" ? key : ""]) : key => var.storage_accounts[key] }
