...
# Perfom Boostrap and rebuild of cluster to new Location (sample: northeurope to westeurope)
...
# RADIX_ZONE_ENV=./radix-zone/radix_zone_dev.env DEST_CLUSTER=weekly-12-recovery ./disaster_recovery.sh
...
if [[ -z "$AZ_LOCATION" ]]; then
    while true; do
        read -p "Migrate to $AZ_LOCATION? (Y/n) " yn
        case $yn in
            [Yy]* ) break;;
...
if [[ ""$(az aks get-credentials --overwrite-existing --admin --resource-group "$AZ_RESOURCE_GROUP_CLUSTERS" --name "$DEST_CLUSTER" 2>&1)"" == *"ARMResourceNotFoundFix"* ]]; then
    ...
    (RADIX_ZONE_ENV="$RADIX_ZONE_ENV" CLUSTER_NAME="$DEST_CLUSTER" USER_PROMPT="$USER_PROMPT" MIGRATION_STRATEGY="$MIGRATION_STRATEGY" AZ_LOCATION="westeurope" source "$BOOTSTRAP_AKS_SCRIPT")
