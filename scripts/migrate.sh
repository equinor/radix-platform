...
###
# Migrate:
# RADIX_ZONE=dev SOURCE_CLUSTER=weekly-32 DEST_CLUSTER=cilium-35 ./migrate.sh > >(tee -a /tmp/stdout.log) 2> >(tee -a /tmp/stderr.log >&2)

# or without log:
# RADIX_ZONE=dev SOURCE_CLUSTER=weekly-32 DEST_CLUSTER=weekly-33 ./migrate.sh
