...
# Example 1:
# RADIX_ZONE_ENV=./../radix-zone/radix_zone_dev.env AUTH_PROXY_COMPONENT="auth" CLUSTER_NAME="weekly-42" RADIX_WEB_CONSOLE_ENV="qa" ./create_redis_cache_for_console.sh

# Example 2:
# RADIX_ZONE_ENV=./../radix-zone/radix_zone_dev.env AUTH_PROXY_COMPONENT="auth" CLUSTER_NAME="weekly-49" RADIX_WEB_CONSOLE_ENV="prod" USER_PROMPT="false" ./create_redis_cache_for_console.sh
...
function createRedisCache() {
    ...
    if [[ $REDIS_CACHE_INSTANCE == "" ]]; then
        ...
        if [[ $USER_PROMPT == true ]]; then
            while true; do
                read -r -p "Do you want to create a new Redis Cache? (Y/n) " yn
                case $yn in
                [Yy]*) break ;;
                [Nn]*)
                    echo "Quitting."
